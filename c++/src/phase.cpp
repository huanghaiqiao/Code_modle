...
static const float abs_erro = 0.001;
...
void compare(Ipp32f* single_dst, cv::Mat& cv_dst)
{
    ...
    for (int x = 0; x < cv_dst.rows; x++)
    {
        ...
        {
            if (single_dst[x * cv_dst.cols + y] < 0) {
                dst = 2 * M_PI + single_dst[x * cv_dst.cols + y];
            }
            else {
                dst = single_dst[x * cv_dst.cols + y];
            }
            //cout << "ipps:  :" << single_dst[x * cv_dst.cols + y] << endl;
            //cout << "opencv: ,tan" << tan(cv_dst.at< float>(x, y)) << endl;

            if (abs(dst - cv_dst.at<float>(x, y)) > abs_erro)
            {
                //cout << "ipps:  :" << dst << endl;
                //cout << "opencv: ,tan" << cv_dst.at<float>(x, y) << endl;
                single_er_num++;
            }
        }
    }
    ...
    //cout << "============ pha ===========" << endl;
    //cout << cv::format(cv_dst, cv::Formatter::FMT_PYTHON) << ";" << endl
    //    << endl
    //    << endl;
    //std::cout << single_er_num << endl;
    std::cout << "absolute error > " << abs_erro << endl;
    std::cout << "accuracy of phase: " << (float)(total_num - single_er_num) / total_num * 100 << "%" << endl;
}
...
int ipp_phase(cv::Mat& img_Re, cv::Mat& img_Im, Ipp32f* pDst) {
    ...
    //status = ippsRealToCplx_32f(pSrc32f_Im, pSrc32f_Re, sDst, cols * rows);

    // 计算sMagnitude
    auto start = std::chrono::high_resolution_clock::now();
    //ippiPhase_32fc32f_C1R, (const Ipp32fc * pSrc, int srcStep, Ipp32f * pDst, int dstStep, IppiSize roiSize))
    for (int i = 0; i < iternum; i++) {
        status = ippiPhase_32fc32f_C1R(sDst, srcStep, pDst, step32, roiSize);
    }
    auto stop = std::chrono::high_resolution_clock::now();
    std::cout << "ippsPhase took "
...
int main() {
    ...
    //cout << "============ pha ===========" << endl;
    //cout << cv::format(pha, cv::Formatter::FMT_PYTHON) << ";" << endl
    //    << endl
    //    << endl;

    ipp_phase(dx, dy, pDst);
    compare(pDst, pha);
    delete[] pDst;
}
